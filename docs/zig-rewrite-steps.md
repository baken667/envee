...
- [x] Шаг 12 — `resolver.zig`: `discover` (подъём по дереву, файлы профиля, `envee.d/*.toml`), `loadAll` (слияние, `stop_search_up`, полный список sources). 171 тест. **Исправлен баг Go: приоритет конфигов был вывернут наизнанку — см. раздел «Найдено в Go».** Коммит будет ниже. 2026-09-09
