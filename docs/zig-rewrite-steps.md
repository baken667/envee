...
- [x] Шаг 16 — `cli/context.zig` (общий контекст, `TrustGate`, путь «найти → проверить доверие → применить»), `cli/resolve.zig` (`resolve` text/json, `diff` text/json), `cli/check.zig` (все правила, `--strict`, `--json`), `cli/test_harness.zig`. 265 тестов, parity 21/21. **Найден пятый дефект Go: `check` не видит цикл в форме `{{env.A}}` — см. «Найдено в Go».** 2026-09-09
  - `check` сверен с Go ПОБАЙТНО в трёх режимах (обычный, `--strict`, `--json`) на всех примерах — 12/12 совпадений. Он не требует доверия, поэтому сверяется целиком уже сейчас.
  - `resolve` и `diff` требуют доверия: их сверка включается вместе с шагом 17.
  - **Найдено по ходу:** подсказка к отсутствующему `_.file` в Go печатает ПОЛНЫЙ путь, а не то, что написано в конфиге. Повторено.
- [x] Шаг 17 — `trust/store.zig` (записи v2, TTL, deny по пути, атомарная запись 0600, RFC3339 вручную), `trust/summary.zig`, `cli/trust.zig` (команды `trust`/`deny`, настоящий гейт, `Asker` вместо чтения stdin в тестах). 301 тест, parity 49/49 при 10 оформленных ожидаемых расхождениях. **Найден шестой баг Go: `resolve`/`diff` игнорируют `profile` из конфига — см. «Найдено в Go».** 2026-09-09
  - **ВЕХА ДОСТИГНУТА.** `envee trust --yes && envee eval bash` работает от начала до конца; бинарь пригоден для ежедневного использования.
  - Включена полная сверка `eval` для 5 оболочек × 4 примера и `resolve --json`. Строка присваивания `$PATH` исключена из сравнения (Go её дублирует) — вместо неё отдельная проверка, что Zig не дублирует.
...
- [x] Шаг 20 — `envee-plugin-env` на Zig: `src/plugins/env.zig` (логика и тесты, чистая `run(argv, stdin, environ) → код`), `src/envee_plugin_env.zig` (main), общий `src/secret_store.zig` для CLI и плагина (путь как у Go-плагина: `$XDG_DATA_HOME` или `~/.local/share`, а не `paths.data()` — файл читают две программы, и это место должно совпадать с уже существующими файлами пользователей). 345 тестов. Parity: Go-ядро + Zig-плагин, Zig-ядро + Zig-плагин и один и тот же E004 на отсутствующий обязательный секрет. Демо-плагин Go SDK резолвится Zig-ядром (тест, пропускается без `go`). 2026-09-09
  - **Отступление от плана:** main лежит в `src/envee_plugin_env.zig`, а не в `src/plugins/env_main.zig`: корень модуля задаёт каталог, за пределы которого `@import` не выходит, а плагину нужны `secret_store.zig` и `trust/store.zig`.
  - **Найдено по ходу, у себя:** E004 печатался не как в Go (`variable`/`detail` в контексте вместо `source`/`ref` и `CAUSE:` со словами плагина). В `errs.Diag` добавлен `cause_text` — текстовая причина для ошибок, пришедших извне.
- [x] Шаг 21 — `cli/status.zig` (`status` со всеми флагами, `doctor`, `daemon status` с настоящим подключением к сокету), `cli/plugin_cmd.zig` (`plugin list/info`, обнаружение с handshake), `cli/exec.zig` (spawn с наследуемыми потоками, код выхода ребёнка через `Ctx.exit_code`), `cli/completion.zig` (bash/zsh/fish/powershell из дерева команд), скрытые команды — E014 с подсказками Go. 355 тестов. Parity: 89 проверок, в том числе `status`/`doctor`/`plugin`/`daemon` побайтно (бинари собираются с одними версией, коммитом и датой), состав команд в справке для `envee`, `plugin`, `daemon`, `secret`. 2026-09-09
  - **Осознанное расхождение:** `completion tcsh` — Go печатает голую строку без кода, здесь E003 с подсказкой; в parity оформлено как `expect_diff`.
  - **Найдено по ходу, в parity-скрипте:** `--` внутри проверяемой команды спутывал разделитель самого `check`; exec заворачивается в `sh -c`.
...
| Вопрос | Когда решать | Дефолт, если владелец не ответил | Как решено |
|---|---|---|---|
| Свой TOML vs `sam701/zig-toml` | конец шага 10 | свой; fallback на библиотеку | свой (шаг 10) |
| yaml в `_.file` | шаг 13 | не поддерживать, понятная ошибка | по дефолту (шаг 13) |
| Судьба `pkg/sdk-go` | шаг 23 | оставить в подкаталоге со своим `go.mod` | по дефолту (шаг 23); перенос в отдельный репозиторий — когда владелец решит |
| Windows-поддержка | шаг 22 | собирать, не тестировать, пометить experimental | по дефолту (шаг 22) |
| Формат `version` в trust-entry v2 | шаг 17 | `2`, v1 читается как Unknown | по дефолту (шаг 17, ADR-0020) |
| Фильтры `json`/`base64` (ADR-0011) | шаг 23 | реализовать | реализованы (шаг 23) |
| `enveed` | шаг 23 | не собирать | не собирается; `daemon status` остался (ADR-0019) |

## Найдено в Go (для переноса правильного поведения)

### ⚠️ `resolve` и `diff` игнорируют `profile` из конфига (`internal/cli/resolve.go`, `diff.go`)

**Симптом.** `eval` вычисляет активный профиль как «флаг → $ENVEE_PROFILE →
`profile` из конфига». `resolve` и `diff` останавливаются на переменной
окружения и в `directive.Apply` передают пустую строку. То есть `resolve`
показывает НЕ ТО окружение, которое применит `eval`, — ровно то, ради чего
его и запускают.

**Проверено на выпущенной версии** в `examples/basic` (там `profile = "dev"`):

```
$ envee eval bash    | grep LOG_PATH
export LOG_PATH=.../logs/dev.log;
$ envee resolve      | grep LOG_PATH
LOG_PATH=.../logs/.log
$ envee --profile dev resolve | grep LOG_PATH
LOG_PATH=.../logs/dev.log
```

**В Zig** обе команды идут общим путём `context.resolveEnv`, где профиль
доопределяется из конфига один раз для всех. Зафиксировано в parity как
ожидаемое расхождение `Go resolve ignores profile= from the config`.

### `check` не видит цикл, записанный как `{{env.A}}` (`internal/cli/check_impl.go`)

**Симптом.** Поиск циклов идёт по регулярному выражению
`\{\{\s*([A-Za-z_][A-Za-z0-9_]*)\s*(\||\}\})`, которое не соответствует
записи `{{env.A}}` из-за точки. При этом именно эта форма документирована и
используется в примерах (`examples/basic`: `{{env.GITHUB_SHA | default(...)}}`).

**Проверено на выпущенной версии:**

| Конфиг | `envee check` | `envee eval` |
|---|---|---|
| `A = "{{ B }}"`, `B = "{{ A }}"` | E003, цикл найден | E007 |
| `A = "{{env.B}}"`, `B = "{{env.A}}"` | «no problems found», код 0 | E007 |

То есть `check` пропускает конфиг, который заведомо не работает, — ровно то,
ради чего его запускают.

**В Zig** ссылки достаются тем же разбором, что и при вычислении
(`template.extractVarRefs`), поэтому видны обе формы. Покрыто тестом
`template cycles are found in both spellings`.
